...
public class BasicController {
    ...
    public String major(
            @RequestParam("stu") String stu // @RequestParam("stu") stu는 url(/spring/major?stu=kim&major=business&grade=3)과 똑같이 적어야한다 (stu = stu)
            , @RequestParam("major") String major // 변수 이름은 다르게 적어도 상관없다
    ...
    // 쿼리 파라미터 키값과 커맨드객체 필드명이 같아야 인식한다
    // (/spring/order?oNum=22&goods=구두&amount=3&price=10000)의 키값과 Order 클래스의 필드명이 같아야 인식한다
    // 반드시 setter/ getter가 있어야 한다
